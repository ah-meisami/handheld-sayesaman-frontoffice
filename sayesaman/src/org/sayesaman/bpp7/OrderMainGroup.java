...
@NoTitle
@Fullscreen
public class OrderMainGroup extends Activity {
    ...
    AdapterSpinnerBuyType adapterSpinnerBuyType;
    @ViewById
    Spinner bpp7_spinner_buytype;
    //==============================================================================================================
    ...
    protected void after() {
        ...
        //==============================================================================================================
        bpp7_spinner_buytype.setVisibility(View.VISIBLE);
        Customer customer = customerDao.getCustomerSpecById(formId);
        bpp7_header2.setText(customer.getCode() + " : " + customer.getName());

        buyTypes = buyTypeDao.getListByCustLevel(customer.getCustLevelRef());
        adapterSpinnerBuyType = new AdapterSpinnerBuyType(this, buyTypes);
        bpp7_spinner_buytype.setAdapter(adapterSpinnerBuyType);

        if (customer.getBuyTypeRef().trim().equals(""))
            customer.setBuyTypeRef(buyTypeDao.getDefaultByCustLevel(customer.getCustLevelRef()));

        bpp7_spinner_buytype.setSelection(adapterSpinnerBuyType.getPositionById(customer.getBuyTypeRef()));


        bpp7_spinner_buytype.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                BuyType buyType = adapterSpinnerBuyType.getObjByPosition(i);
                orderDao.saveBuyType(formId, buyType);
            }
    ...
    private View getNewView(int i) {
        ...
        OrderStatus status = goodsGroupDao.getOrderStatus4MainGoodsGroup(formId, group.getId());
        ((TextView) view.findViewById(R.id.goodsQtyRequest)).setText(status.getOrder_qty());
        ((TextView) view.findViewById(R.id.goodsQtyRequestSum)).setText(status.getOrder_sum());
        ((TextView) view.findViewById(R.id.goodsQtyRequestSumPrice)).setText(Statics.commas.insertCommas(status.getOrder_price()));
        ...
        view.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent i = new Intent();
                i.setClass(getApplicationContext(), org.sayesaman.bpp7.OrderSubGroup_.class);
    ...
    private void fillFooter() {
        OrderStatus status = goodsGroupDao.getOrderStatusById(formId);
        String countGoods = goodsGroupDao.getCountGoods4NewOrder();
