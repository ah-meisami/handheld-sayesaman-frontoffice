...
import org.sayesaman.database.dao.BuyTypeDao;
 ...
 */
@EActivity(R.layout.bpp7_order)
@NoTitle
@Fullscreen
public class OrderGoods extends Activity {
    ...
    @Bean
    BuyTypeDao buyTypeDao;
    ArrayList<BuyType> buyTypes = new ArrayList<BuyType>();
    ...
    private void initOrder() {
        orderItems = orderDao.getOrderItemsForNewOrder(formId, groupId1, groupId2);
        if (orderItems.size() == 0) {
            ...
            for (int i = 0; i < orderItems.size(); i++) {
                AsyncTaskRunnerOrder runner = new AsyncTaskRunnerOrder();
    ...
    private class AsyncTaskRunnerOrder extends AsyncTask<String, Void, View> {
        @Override
        protected View doInBackground(String... params) {
            ...
            */
            return getViewOrder(Integer.valueOf(params[0]));
        }
    ...
    private View getViewOrder(int i) {
        View view = inflater.inflate(R.layout.bpp7_order_row, null, false);
        Goods goods = orderItems.get(i).getGoods();
        //*******************************************************************************************
        if (i % 2 == 0) {
        ...
        TextView tvGoodsId = (TextView) view.findViewById(R.id.bpp7_row_goods_id);
        TextView tvGoodsCode = (TextView) view.findViewById(R.id.bpp7_row_goods_code);
        TextView tvGoodsImage = (TextView) view.findViewById(R.id.bpp7_row_goods_name);
        ...
        //*******************************************************************************************
        tvNo.setText(orderItems.get(i).getNo());
        tvId.setText(orderItems.get(i).getId());
        tvGoodsId.setText(goods.getId());
        tvGoodsCode.setText(goods.getCode());
        tvGoodsImage.setText(goods.getName());
        tvQty.setText(Statics.commas.insertCommas(orderItems.get(i).getQty()));
        tvPrice.setText(orderItems.get(i).getPrice());
        tvPriceSum.setText(Statics.commas.insertCommas(orderItems.get(i).getPrice_sum()));
        Statics.setImg2(ivGoods, "Goods", goods.getCode());

        if (Long.parseLong(orderItems.get(i).getQty()) > 0) {
        ...
        ((Button) view.findViewById(R.id.bpp7_row_btn_plus)).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (parent != null) {
                    ...
                    long y = Long.parseLong(String.valueOf(qty)) * Long.parseLong(tvPrice.getText().toString().replace(".0", ""));
                    tvPriceSum.setText(Statics.commas.insertCommas(String.valueOf(y)).replace(".0", ""));

                    String orderItmId = orderDao.saveItem(formId, tvId.getText().toString(), tvGoodsRef.getText().toString(), tvQty.getText().toString());
                    tvId.setText(orderItmId);
                }
                fillFooter();
            }
        });
        ...
        ivGoods.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (parent != null) {
                    ...
                    final Dialog dialog = new Dialog(OrderGoods.this);
        ...
        if (requestCode == 1) {
            if (resultCode == RESULT_OK) {
                ...
                if (oprType.equals("order")) {
                    ...
                    long y = Long.parseLong(String.valueOf(numQty)) * Long.parseLong(tvPrice.getText().toString().replace(".0", ""));
                    tvPriceSum.setText(Statics.commas.insertCommas(String.valueOf(y)).replace(".0", ""));
    ...
    private void fillFooter() {
        if (formType.equals("order")) {
            bpp7_footer1.setText(String.valueOf(orderItems.size()));
        } else {
            bpp7_footer1.setText(String.valueOf(damageItems.size()));
        }
        bpp7_footer2.setText("<< مجموع >>");
        OrderStatus status = goodsGroupDao.getOrderStatusByIdAndMainGoodsGroupAndSubGoodsGroup(formId, groupId1, groupId2);
        bpp7_footer3.setText(Statics.commas.insertCommas(status.getOrder_sum()));
        bpp7_footer4.setText(Statics.commas.insertCommas(status.getOrder_price()));
    }
}