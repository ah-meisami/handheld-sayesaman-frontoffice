 ...
 * @author BEHNAM
 */
public class MyClanedar {

    public MyClanedar() {
    }

    double J2000 = 2451545.0, // Julian day of J2000 epoch
...
//  DCOS  --  Cosine of an angle in degrees
    ...
    /*  MOD  --  Modulus public double  which works for non-integers.  */
    public int mod(double a, double b) {
        return (int) (a - (b * Math.floor(a / b)));
    }

//  AMOD  --  Modulus public double  which returns numerator if modulus is zero
    public int amod(double a, double b) {
        return mod(a - 1, b) + 1;
    }
...
//  JWDAY  --  Calculate day of week from Julian day
    ...
    public double[] nutation(double jd) {
        ...
        /* Range reduce the angles in case the sine and cosine public double s
    ...
    public double deltat(double year) {
        double dt, f, t;
        int i;
...
//  Periodic terms to obtain true time
    double EquinoxpTerms[] = new double[]{
        485, 324.96, 1934.136,
    ...
    public double equinox(double year, int which) {
        double deltaL;
        int i, j;
        double JDE0, JDE, S, T, W, Y;
        double JDE0tab[][];
...
//System.out.println("debug.log.value += "JDE0 = " + JDE0 + "\n";

        T = (JDE0 - 2451545.0) / 36525;
//System.out.println("debug.log.value += "T = " + T + "\n";
        W = (35999.373 * T) - 2.47;
//System.out.println("debug.log.value += "W = " + W + "\n";
        deltaL = 1 + (0.0334 * dcos(W)) + (0.0007 * dcos(2 * W));
//System.out.println("debug.log.value += "deltaL = " + deltaL + "\n";
...
//System.out.println("debug.log.value += "S = " + S + "\n";
//System.out.println("debug.log.value += "Corr = " + ((S * 0.00001) / deltaL) + "\n";
    ...
    on the return statement at the end of this public double 
...
//System.out.println("debug.log.value += "Sunpos.  T = " + T + "\n";
        T2 = T * T;
        L0 = 280.46646 + (36000.76983 * T) + (0.0003032 * T2);
//System.out.println("debug.log.value += "L0 = " + L0 + "\n";
        L0 = fixangle(L0);
//System.out.println("debug.log.value += "L0 = " + L0 + "\n";
        M = 357.52911 + (35999.05029 * T) + (-0.0001537 * T2);
//System.out.println("debug.log.value += "M = " + M + "\n";
        M = fixangle(M);
//System.out.println("debug.log.value += "M = " + M + "\n";
        e = 0.016708634 + (-0.000042037 * T) + (-0.0000001267 * T2);
//System.out.println("debug.log.value += "e = " + e + "\n";
        C = ((1.914602 + (-0.004817 * T) + (-0.000014 * T2)) * dsin(M))
                + ((0.019993 - (0.000101 * T)) * dsin(2 * M))
                + (0.000289 * dsin(3 * M));
//System.out.println("debug.log.value += "C = " + C + "\n";
        sunLong = L0 + C;
//System.out.println("debug.log.value += "sunLong = " + sunLong + "\n";
        sunAnomaly = M + C;
//System.out.println("debug.log.value += "sunAnomaly = " + sunAnomaly + "\n";
        sunR = (1.000001018 * (1 - (e * e))) / (1 + (e * dcos(sunAnomaly)));
//System.out.println("debug.log.value += "sunR = " + sunR + "\n";
        Omega = 125.04 - (1934.136 * T);
//System.out.println("debug.log.value += "Omega = " + Omega + "\n";
        Lambda = sunLong + (-0.00569) + (-0.00478 * dsin(Omega));
//System.out.println("debug.log.value += "Lambda = " + Lambda + "\n";
        epsilon0 = obliqeq(jd);
//System.out.println("debug.log.value += "epsilon0 = " + epsilon0 + "\n";
        epsilon = epsilon0 + (0.00256 * dcos(Omega));
//System.out.println("debug.log.value += "epsilon = " + epsilon + "\n";
        Alpha = rtd(Math.atan2(dcos(epsilon0) * dsin(sunLong), dcos(sunLong)));
//System.out.println("debug.log.value += "Alpha = " + Alpha + "\n";
        Alpha = fixangle(Alpha);
////System.out.println("debug.log.value += "Alpha = " + Alpha + "\n";
        Delta = rtd(Math.asin(dsin(epsilon0) * dsin(sunLong)));
////System.out.println("debug.log.value += "Delta = " + Delta + "\n";
        AlphaApp = rtd(Math.atan2(dcos(epsilon) * dsin(Lambda), dcos(Lambda)));
//System.out.println("debug.log.value += "AlphaApp = " + AlphaApp + "\n";
        AlphaApp = fixangle(AlphaApp);
//System.out.println("debug.log.value += "AlphaApp = " + AlphaApp + "\n";
        DeltaApp = rtd(Math.asin(dsin(epsilon) * dsin(Lambda)));
//System.out.println("debug.log.value += "DeltaApp = " + DeltaApp + "\n";

        return new double[]{ //  Angular quantities are expressed in decimal degrees
...
//System.out.println("debug.log.value += "equationOfTime.  tau = " + tau + "\n";
...
//System.out.println("debug.log.value += "deltaPsi = " + deltaPsi + "\n";
        epsilon = obliqeq(jd) + nutation(jd)[1];
//System.out.println("debug.log.value += "epsilon = " + epsilon + "\n";
        E = L0 + (-0.0057183) + (-alpha) + (deltaPsi * dcos(epsilon));
//System.out.println("debug.log.value += "E = " + E + "\n";
        E = E - 20.0 * (Math.floor(E / 20.0));
//System.out.println("debug.log.value += "Efixed = " + E + "\n";
        E = E / (24 * 60);
//System.out.println("debug.log.value += "Eday = " + E + "\n";
    ...
    double J1900 = 2415020.5;                // Epoch (day 1) of Excel 1900 date system (PC)
    double J1904 = 2416480.5;                // Epoch (day 0) of Excel 1904 date system (Mac)
    String NormLeap[] = new String[]{"Normal year", "Leap year"};

    /*  WEEKDAY_BEFORE  --  Return Julian date of given weekday (0 = Sunday)
    in the seven days ending on jd.  */
    public double weekday_before(double weekday, double jd) {
     ...
     */
    public double search_weekday(double weekday, double jd, double direction, double offset) {
        return weekday_before(weekday, jd + (direction * offset));
    }

//  Utility weekday functions, just wrappers for search_weekday
    public double nearest_weekday(double weekday, double jd) {
        return search_weekday(weekday, jd, 1, 3);
    }

    public double next_weekday(double weekday, double jd) {
        return search_weekday(weekday, jd, 1.0, 7.0);
    }

    public double next_or_current_weekday(double weekday, double jd) {
        return search_weekday(weekday, jd, 1, 6);
    }

    public double previous_weekday(double weekday, double jd) {
        return search_weekday(weekday, jd, -1, 1);
    }

    public double previous_or_current_weekday(double weekday, double jd) {
...
//  GREGORIAN_TO_JD  --  Determine Julian day number from Gregorian calendar date
    ...
    public double gregorian_to_jd(double year, double month, double day) {
...
//  JD_TO_GREGORIAN  --  Calculate Gregorian calendar date from Julian day
    public double[] jd_to_gregorian(double jd) {
...
//  ISO_TO_JULIAN  --  Return Julian day of given ISO year, week, and day
    public double n_weeks(double weekday, double jd, double nthweek) {
        double j = 7 * nthweek;
    ...
    public double iso_to_julian(double year, double week, double day) {
        return day + n_weeks(0, gregorian_to_jd(year - 1, 12, 28), week);
    }

//  JD_TO_ISO  --  Return array of ISO (year, week, day) for Julian day
    public double[] jd_to_iso(double jd) {
        double year, week, day;
...
//  JULIAN_TO_JD  --  Determine Julian day number from Julian calendar date
    double JULIAN_EPOCH = 1721423.5;

    public boolean leap_julian(double year) {
        return (year % 4) == ((year > 0) ? 0 : 3);
    }

    public double julian_to_jd(double year, double month, double day) {
...
//  JD_TO_JULIAN  --  Calculate Julian calendar date from Julian day
    public double[] jd_to_julian(double td) {
        double z, a, alpha, b, c, d, e, year, month, day;
...
//  HEBREW_TO_JD  --  Determine Julian day from Hebrew date
    double HEBREW_EPOCH = 347995.5;

//  Is a given Hebrew year a leap year ?
    public boolean hebrew_leap(double year) {
        return mod(((year * 7) + 1), 19) < 7;
    }

//  How many months are there in a Hebrew year (12 = normal, 13 = leap)
    public int hebrew_year_months(double year) {
        return hebrew_leap(year) ? 13 : 12;
    }
//  Test for delay of start of new year and to avoid
//  Sunday, Wednesday, and Friday as start of the new year.

    public double hebrew_delay_1(double year) {
...
//  Check for delay in start of new year due to length of adjacent years
    public double hebrew_delay_2(double year) {
        double last, present, next;
...
//  How many days are in a Hebrew year ?
    public double hebrew_year_days(double year) {
        return hebrew_to_jd(year + 1, 7, 1) - hebrew_to_jd(year, 7, 1);
    }

//  How many days are in a given month of a given year
    public double hebrew_month_days(double year, double month) {
...
//  Finally, wrap it all up into...
    public double hebrew_to_jd(double year, double month, double day) {
        double jd, mon, months;
    ...
    slow.  */
    public double[] jd_to_hebrew(double jd) {
        double year, month, day, i, count, first;
    ...
    public double equinoxe_a_paris(double year) {
    ...
    year.  */
    public double paris_equinoxe_jd(double year) {
        double ep, epg;
     ...
     */
    double FRENCH_REVOLUTIONARY_EPOCH = 2375839.5;

    public double[] annee_da_la_revolution(double jd) {
    ...
    public double[] jd_to_french_revolutionary(double jd) {
    ...
    public double french_revolutionary_to_jd(double an, double mois, double decade, double jour) {
        double adr[], equinoxe, guess, jd;

        guess = FRENCH_REVOLUTIONARY_EPOCH + (TropicalYear * ((an - 1) - 1));
        adr = new double[]{an - 1, 0};
...
//  JD_TO_ISLAMIC  --  Calculate Islamic date from Julian day
    public double[] jd_to_islamic(double jd) {
...
//   LEAP_KURDISH  --- Is a given year a leap year in the Kurdish calendar
    public boolean leap_kurdish(double year) {
        return ((((((year - ((year > 0) ? 474 : 473)) % 2820) + 474) + 38) * 682) % 2816) < 682;
    }
//  KURDISH_TO_JD  --  Determine Julian day from Kurdish date 1497976.5
    double KURDISH_EPOCH = 1497976.5;
    String KURDISH_WEEKDAYS[] = new String[]{"Yeksheme", "Dusheme",
        "Sésheme", "Ciwarsheme",
        "Penjsheme", "Héní", "Sheme"};

    public double kurdish_to_jd(double year, double month, double day) {
        double kuepbase, kuepyear;

        kuepbase = year - ((year >= 0) ? 474 : 473);
        kuepyear = 474 + mod(kuepbase, 2820);

        return day
                + ((month <= 7)
                ? ((month - 1) * 31)
                : (((month - 1) * 30) + 6))
                + Math.floor(((kuepyear * 682) - 110) / 2816)
                + (kuepyear - 1) * 365
                + Math.floor(kuepbase / 2820) * 1029983
                + (KURDISH_EPOCH - 1);
    }

//  JD_TO_KURDISH  --  Calculate Kurdish date from Julian day
    public double[] jd_to_kurdish(double jd) {
        double year, month, day, kudepoch, kucycle, kucyear, ykucycle,
                kuaux1, kuaux2, kuyday;


        jd = Math.floor(jd) + 0.5;

        kudepoch = jd - kurdish_to_jd(475, 1, 1);
        kucycle = Math.floor(kudepoch / 1029983);
        kucyear = mod(kudepoch, 1029983);
        if (kucyear == 1029982) {
            ykucycle = 2820;
        } else {
            kuaux1 = Math.floor(kucyear / 366);
            kuaux2 = mod(kucyear, 366);
            ykucycle = Math.floor(((2134 * kuaux1) + (2816 * kuaux2) + 2815) / 1028522)
                    + kuaux1 + 1;
        }
        year = ykucycle + (2820 * kucycle) + 474;
        if (year <= 0) {
            year--;
        }
        kuyday = (jd - kurdish_to_jd(year, 1, 1)) + 1;
        month = (kuyday <= 186) ? Math.ceil(kuyday / 31) : Math.ceil((kuyday - 6) / 30);
        day = (jd - kurdish_to_jd(year, month, 1)) + 1;
        return new double[]{year, month, day};
    }
...
//  PERSIAN_TO_JD  --  Determine Julian day from Persian date
    double PERSIAN_EPOCH = 1948320.5;
    String PERSIAN_WEEKDAYS[] = new String[]{"Yekshanbeh", "Dushanbeh",
        "Seshanbeh", "Cháhárshanbeh",
        "Panjshanbeh", "Jomeh", "Shanbeh"};

    public double persian_to_jd(double year, double month, double day) {
...
//  JD_TO_PERSIAN  --  Calculate Persian date from Julian day
    public double[] jd_to_persian(double jd) {
...
//  MAYAN_COUNT_TO_JD  --  Determine Julian day from Mayan long count
    ...
    public double mayan_count_to_jd(double baktun, double katun, double tun, double uinal, double kin) {
...
//  JD_TO_MAYAN_COUNT  --  Calculate Mayan long count from Julian day
    public double[] jd_to_mayan_count(double jd) {
        ...
        return new double[]{baktun, katun, tun, uinal, kin};
    }
//  JD_TO_MAYAN_HAAB  --  Determine Mayan Haab "month" and day from Julian day
    String MAYAN_HAAB_MONTHS[] = new String[]{"Pop", "Uo", "Zip", "Zotz", "Tzec", "Xul",
        "Yaxkin", "Mol", "Chen", "Yax", "Zac", "Ceh",
        "Mac", "Kankin", "Muan", "Pax", "Kayab", "Cumku"};

    public double[] jd_to_mayan_haab(double jd) {
        double lcount, day;

        lcount = jd - MAYAN_COUNT_EPOCH;
        day = mod(lcount + 8 + ((18 - 1) * 20), 365);

        return new double[]{Math.floor(day / 20) + 1, mod(day, 20)};
    }
//  JD_TO_MAYAN_TZOLKIN  --  Determine Mayan Tzolkin "month" and day from Julian day
    String MAYAN_TZOLKIN_MONTHS[] = new String[]{"Imix", "Ik", "Akbal", "Kan", "Chicchan",
        "Cimi", "Manik", "Lamat", "Muluc", "Oc",
        "Chuen", "Eb", "Ben", "Ix", "Men",
        "Cib", "Caban", "Etxnab", "Cauac", "Ahau"};

    public double[] jd_to_mayan_tzolkin(double jd) {
        double lcount = jd - MAYAN_COUNT_EPOCH;

        return new double[]{amod(lcount + 20, 20), amod(lcount + 4, 13)};
    }
//  BAHAI_TO_JD  --  Determine Julian day from Bahai date
    double BAHAI_EPOCH = 2394646.5;
    String BAHAI_WEEKDAYS[] = new String[]{"Jamál", "Kamál", "Fidál", "Idál",
        "Istijlál", "Istiqlál", "Jalál"};

    public double bahai_to_jd(double major, double cycle, double year, double month, double day) {
        double gy;
...
//  JD_TO_BAHAI  --  Calculate Bahai date from Julian day
    public double[] jd_to_bahai(double jd) {
        double major, cycle, year, month, day,
        ...
        return new double[]{major, cycle, year, month, day};
    }
//  INDIAN_CIVIL_TO_JD  --  Obtain Julian day for Indian Civil date
    String INDIAN_CIVIL_WEEKDAYS[] = new String[]{
        "ravivara", "somavara", "mangalavara", "budhavara",
        "brahaspativara", "sukravara", "sanivara"};

    public double indian_civil_to_jd(double year, double month, double day) {
        double Caitra, gyear;
        boolean leap;
        double start, jd, m;
...
//  JD_TO_INDIAN_CIVIL  --  Calculate Indian Civil date from Julian day
    public double[] jd_to_indian_civil(double jd) {
        double Caitra, Saka, greg[], greg0;
        boolean leap;
        double start, year, yday, mday;
    ...
    calendars.  */
    public void main(double year,double mon,double mday) {
        double j, hour, min, sec;
        int weekday;
        double julcal[], islcal[], kurscal[];
        hour = min = sec = 0;
	/////agar tarikh milady bashe bayad az in tabe estefade koni
	//j=gregorian_to_jd(year, mon, mday);
	//va baraye tabdile miladi be shamsi mituni az tabe
	////jd_to_persian(j); ke in tabe ye araye barmigardune ke in araye 
	///khunehash be tartib sal,mah,ruz khahad bud
	///////baghyeye tarikh ha ro ham mituni dar biari
        //  Update Julian day
        j=persian_to_jd(year, mon, mday);
//        j = gregorian_to_jd(year, mon + 1, mday)
//                + ((sec + 60 * (min + 60 * hour)) / 86400.0);

        System.out.println("julianday.day.value=" + j);
        System.out.println("modifiedjulianday.day.value = " + (j - JMJD));

        //  Update day of week in Gregorian box

        weekday = jwday(j);
        System.out.println("gregorian.wday.value = " + Weekdays[weekday]);

        //  Update leap year status in Gregorian box

        System.out.println("gregorian.leap.value =" + NormLeap[leap_gregorian(year) ? 1 : 0]);

        //  Update Julian Calendar
        
        julcal = jd_to_julian(j);
        System.out.println("juliancalendar.year.value = " + julcal[0]);
        System.out.println("juliancalendar.month.selectedIndex = " + (julcal[1]));
        System.out.println("juliancalendar.day.value = " + julcal[2]);
        System.out.println("juliancalendar.leap.value = " + NormLeap[leap_julian(julcal[0]) ? 1 : 0]);
        ...
        System.out.println("islamic.year.value = " + islcal[0]);
        System.out.println("islamic.month.selectedIndex = " + (islcal[1]));
        System.out.println("islamic.day.value = " + islcal[2]);
        System.out.println("islamic.wday.value = " + "yawm " + ISLAMIC_WEEKDAYS[weekday]);
        System.out.println("islamic.leap.value = " + NormLeap[leap_islamic(islcal[0]) ? 1 : 0]);

        //  Update Kurdish Calendar

        kurscal = jd_to_kurdish(j);
        System.out.println("kurdish.year.value = " + kurscal[0]);
        System.out.println("kurdish.month.selectedIndex = " + (kurscal[1]));
        System.out.println("kurdish.day.value = " + kurscal[2]);
        System.out.println("kurdish.wday.value = " + KURDISH_WEEKDAYS[weekday]);
        System.out.println("kurdish.leap.value = " + NormLeap[leap_kurdish(kurscal[0]) ? 1 : 0]);


        //  Update Persian Calendar

        double perscal[] = jd_to_persian(j);
        System.out.println("persian.year.value = " + perscal[0]);
        System.out.println("persian.month.selectedIndex = " + (perscal[1]));
        System.out.println("persian.day.value = " + perscal[2]);
        System.out.println("persian.wday.value = " + PERSIAN_WEEKDAYS[weekday]);
        System.out.println("persian.leap.value = " + NormLeap[leap_persian(perscal[0]) ? 1 : 0]);
    }

    MyDate shtom(MyDate d) {
        double j;
        double result[];

        //  Update Julian day
        j=persian_to_jd(d.year, d.month, d.day);
        result = jd_to_gregorian(j);
        System.out.println(d+"::::"+new MyDate((int)result[0],(int) result[1],(int) result[2]));
        return new MyDate((int)result[0],(int) result[1],(int) result[2]);

    }
    int getfd(double year){
        double j=persian_to_jd(year,1,1);
        return jwday((j+1)%7);
    }
//    ghamariDate shtogh(Date d) {
//        double j;
//        double result[];
//        //  Update Julian day
//        j=persian_to_jd(d.year, d.month, d.day);
//        result = jd_to_islamic(j);
//        System.out.println(d+"::"+new ghamariDate((int)result[2],(int) result[1],(int) result[0]));
//        return new ghamariDate((int)result[2],(int) result[1],(int) result[0]);
//    }
    public MyDate fromMiladiToShamsi(Date d){
    /////agar tarikh milady bashe bayad az in tabe estefade koni
    	//j=gregorian_to_jd(year, mon, mday);
    	//va baraye tabdile miladi be shamsi mituni az tabe
    	////jd_to_persian(j); ke in tabe ye araye barmigardune ke in araye 
    	///khunehash be tartib sal,mah,ruz khahad bud
    	///////baghyeye tarikh ha ro ham mituni dar biari
    	double j=gregorian_to_jd(d.getYear()+1900, d.getMonth()+1, d.getDate());
    	double result[]=jd_to_persian(j);
    	return new MyDate((int)result[0], (int)result[1], (int) result[2]);
    }
